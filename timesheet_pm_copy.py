...
class PM_Timesheet(object):
    def setupUi(self, MainWindow):
        ...
        self.table.setGeometry(QtCore.QRect(50, 300, 1750, 500))         #Size of the Table
        ...
        self.table.setColumnCount(11)  ## Column Count 
        ...
        self.columnwidth = 159
        self.Headers = ["Subcon ID","Subcon Name","Unit","Sub Unit","Month","Customer Name","Customer Group Name","Project ID","Project Description","Request Status", "My Decision"]   #Header Names for the Table
        i = 0
        ...
        self.Remarks.setText("PM Remarks:")
        self.Remarks.setGeometry(QtCore.QRect(630,830,120,35))
        ...
        self.Remarks_value.setGeometry(QtCore.QRect(760,830,220,35))
        ...
        self.Reason.setText("PM Reason:")
        self.Reason.setGeometry(QtCore.QRect(1000,830,120,35))
        self.Reason_value = QtWidgets.QComboBox(self.page_2)       # Line Edit to display PM Remarks
        font = QtGui.QFont()
        font.setPointSize(12)
        self.Reason_value.setFont(font)
        self.Reason_value.addItems(["A","B", "C", "D"])
        self.Reason_value.setObjectName("Remarks_value")
        self.Reason_value.setGeometry(QtCore.QRect(1130,830,150,35))
        ...
        self.Remark_sbu.setGeometry(QtCore.QRect(1310,830,130,35))
        ...
        self.Remark_value_sbu.setGeometry(QtCore.QRect(1450,830,250,35))
    ...
    def calculatevendorhours(self):
        ...
        if vhours != 0.0:
            self.variance_value.setText(str(round((vhours - rus)/vhours,2)) + '%')
            if (vhours - rus)/vhours <= 1.0:
                    self.variance_value.setStyleSheet('background-color: rgb(144,238,144);')
            elif (vhours - rus)/vhours > 1.0 and (vhours - rus)/vhours <= 3.0:
    ...
    def submitprogress(self):
        ...
        if self.chours.text() == "":
            ...
            if variance <= 1: ## If variance is less than 1, then approve the timesheet.
                connection = sqlite3.connect('EY_1')
                cur = connection.cursor()
                cur.execute('update Timesheet_PM set Payment_amount = ?, NonBillable_amount = ?,Variance = ?,Remarks = ?, Reason = ?, Decision = "Approve", Request_Status = "Approved" where Subcon_ID = 2025 and Month = ?', (pamount,nbamount,variance,remarks,reason,self.Month,))
                connection.commit()
                connection.close()
                self.loaddata_1()
            elif variance > 1:  # If variance is greater than 1, then send the request to SBU head
                connection = sqlite3.connect('EY_1')
                cur = connection.cursor()
                cur.execute('update Timesheet_PM set Payment_amount = ?, NonBillable_amount = ?,Variance = ?, Remarks = ?, Reason = ?, Decision = "Approve", Request_Status = "Pending With SBU Head" where Subcon_ID = 2025 and Month = ?', (pamount,nbamount,variance,remarks,reason,self.Month,))
                connection.commit()
                connection.close()
                self.error_dialog = QtWidgets.QMessageBox()
                self.error_dialog.setText("The Variance for the given transaction is greater than 1 and has been sent for SBU Heads' approval")
                self.error_dialog.exec_()
                self.loaddata_1()
            self.stackedWidget.setCurrentIndex(0)
    ...
    def showdata(self):                 ## This function autofills the data into the tablewidget i.e. Timesheet
        ...
        connection = sqlite3.connect('EY_1')
        cur = connection.cursor()
        result = cur.execute('select * from Timesheet_PM where Subcon_ID = 2025 and Month = ? and Year = ?',(self.Month,self.year,))
        k = 0
        for row in result:
            for i in range(7):
                if tablecol <= 6:
    ...
    def loaddata_2(self):
        connection = sqlite3.connect('EY_1')
        cur = connection.cursor()
        result = cur.execute('select * from Timesheet_PM where Subcon_ID = ? and Month = ?',(self.Subcon_ID,self.Month,))
        for row in result:
            self.sid.setText(str(row[0]))
            self.sname.setText(str(row[1]))
            self.unit_s.setText(str(row[2]))
            self.sunit.setText(str(row[3]))
            self.month_name.setText(str(row[4]))
            ...
            if str(row[14]) != "None":
                ...
                self.variance_value.setText(str((row[11] - row[14])/row[14])) ## Calculating Variance 
                ## Highlighting variance based on the value 
                if float(self.variance_value.text()) <= 1.0: 
                    self.variance_value.setStyleSheet('background-color: rgb(144,238,144);')
                elif float(self.variance_value.text()) > 1.0 and float(self.variance_value.text()) <= 3.0:
                    self.variance_value.setStyleSheet('background-color: rgb(255,191,0);')
                else:
                    self.variance_value.setStyleSheet('background-color: rgb(255,0,0);')
            if str(row[15]) != "None":
        ...
        result = cur.execute('select * from Timesheet_PM where Subcon_ID = ?',(self.Subcon_ID,))
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.nrc.setText(_translate("MainWindow", "NRC:"))
