## All the functions associated with EHC_Renewal have the same logic as the functions associated with SBU_Renewal. 
...
class EHC_Renewal(object):
    ...
    def showproject(self):  ## To show project details on click of button
        ...
        self.line_edit = {}  ## The names of the line edits and their positions are mentioned in the list
        line_edit = {
    ...
    def showother(self):  ## Similar logics are used as in showprojectdetails part.
        self.gridLayoutWidget_6.setGeometry(QtCore.QRect(1191, 70, 570, 150))
        self.line_edit_10 = {}
        line_edit_10 = {
                "Customer Group Name": (0,1),	
                "W2 Vendor": (0,3), "Location": (1,1),"Contract Type": (1,3),
            }

        self.lable_10 = {}
        lable_10 = {
                "Customer Group Name": (0,0),	
                "W2 Vendor": (0,2), "Location": (1,0),"Contract Type": (1,2),
        }
    ...
    def Insert1(self): ## On clicking the Submit button, the function inserts/update the info in the Decisions Table.
        ...
        connection = sqlite3.connect("EY")
        cur = connection.cursor()
        if self.Cummulative == "Red": # If Cumulative RAG = Red, Approval or conditional approval implies Closing and Approving of Request
            if self.text_2 == "Approve":
                cur.execute('update Decisions set  Decision_EHC = "Approve", EHC_Remarks = ?, EHC_Extension_Tenure = ?, Request_Status = "Approved", Transaction_Date_EHC = datetime("now") where Subcon_ID = ? and Request_Status = "Pending"', (self.text_1,self.text_3,self.ID,))
            elif self.text_2 == "Conditional Approval":
                cur.execute('update Decisions set  Decision_EHC = "Conditional Approval", EHC_Remarks = ?, EHC_Extension_Tenure = ?, Request_Status = "Approved", Transaction_Date_EHC = datetime("now") where Subcon_ID = ? and Request_Status = "Pending"', (self.text_1,self.text_3,self.ID,))
            elif self.text_2 == "Query":
                cur.execute('update Decisions set Decision_EHC = "Query", EHC_Remarks = ?, EHC_Extension_Tenure = ?, Request_Pending_With = ?, Transaction_Date_EHC = datetime("now") where Subcon_ID = ? and Request_Status = "Pending"', (self.text_1,self.text_3,"SBU",self.ID,))
            else:
                cur.execute('update Decisions set Decision_EHC = "Reject", EHC_Remarks = ?, EHC_Extension_Tenure = ?, Request_Status = "Rejected", Transaction_Date_EHC = datetime("now") where Subcon_ID = ? and Request_Status = "Pending"', (self.text_1,self.text_3,self.ID,))
    ...
    def loaddata_1(self):   ## Loading data into Page 3
        ...
        self.line_edits_1["Tenure"].setText(str(round(tenure/30, 0)))   # Finally we report Tenure in months, so dividing it by 30
        ...
        for row in results:   
            ...
            self.line_edits_3["Tenure RAG"].setText(str(row[28]))
    ...
    def ShowFPLess(self, row1): ## Show the info of all the Subcons under the Category, Contract Type = FP and WO Expiry Date < 30 days
        connection = sqlite3.connect('EY')
        cur = connection.cursor()
        sqlstr = 'SELECT * FROM Subcon_Info, Decisions where Subcon_Info.Contract_Type = "FP" AND Decisions.Decision_EHC = ? And (Julianday(Subcon_Info.Expected_End_Date) - Julianday("now"))  between 0 and 30  AND Decisions.Subcon_ID = Subcon_Info.Subcon_ID AND Subcon_Info.Cummulative_RAG = "Red"'
        
        if row1 == 0: # All the approved and conditionally approved requests
            results = cur.execute('SELECT * FROM Subcon_Info, Decisions where Subcon_Info.Contract_Type = "FP" AND (Decisions.Decision_EHC = "Approve" or Decisions.Decision_EHC = "Conditional Approval") And (Julianday(Subcon_Info.Expected_End_Date) - Julianday("now"))  between 0 and 30  AND Decisions.Subcon_ID = Subcon_Info.Subcon_ID AND Subcon_Info.Cummulative_RAG = "Red"')
        elif row1 == 1: # All the rejected requests
            results = cur.execute(sqlstr,("Reject",))
        elif row1 == 2: # All the sent back requests
            results = cur.execute(sqlstr,("Query",))
        elif row1 == 3: # All the unattended requests under EHC Approver
            results = cur.execute('select * from Subcon_Info, Decisions where Decisions.Request_Pending_With = "EHC" and Decisions.Request_Status = "Pending" and Decisions.Decision_EHC IS NULL and Subcon_Info.Contract_Type = "FP" and (Julianday(Subcon_Info.Expected_End_Date) - Julianday("now"))  between 0 and 30  and Decisions.Subcon_ID = Subcon_Info.Subcon_ID  and Subcon_Info.Cummulative_RAG = "Red"')
        else:   # all the requests under EHC Approver
            results = cur.execute('select * from Subcon_Info, Decisions where (Decisions.Decision_SBU = "Approve" or Decisions.Decision_SBU = "Conditional Approval") and Subcon_Info.Contract_Type = "FP" and (Julianday(Subcon_Info.Expected_End_Date) - Julianday("now"))  between 0 and 30  and Decisions.Subcon_ID = Subcon_Info.Subcon_ID  and Subcon_Info.Cummulative_RAG = "Red"')
        tablerow=0
        for row in results:
            for i in range(0,32):
    ...
    def ShowTMLess(self, row1): ## Show the info of all the Subcons under the Category, Contract Type = T&M and WO Expiry Date < 30 days
        connection = sqlite3.connect('EY')
        cur = connection.cursor()
        sqlstr = 'SELECT * FROM Subcon_Info, Decisions where Subcon_Info.Contract_Type = "T&M" AND Decisions.Decision_EHC = ? And (Julianday(Subcon_Info.Expected_End_Date) - Julianday("now"))  between 0 and 30  AND Decisions.Subcon_ID = Subcon_Info.Subcon_ID  and Subcon_Info.Cummulative_RAG = "Red"'
        tablerow=0
        if row1 == 0:
            results = cur.execute('SELECT * FROM Subcon_Info, Decisions where Subcon_Info.Contract_Type = "T&M" AND (Decisions.Decision_EHC = "Approve" or Decisions.Decision_EHC = "Conditional Approval") And (Julianday(Subcon_Info.Expected_End_Date) - Julianday("now"))  between 0 and 30  AND Decisions.Subcon_ID = Subcon_Info.Subcon_ID  and Subcon_Info.Cummulative_RAG = "Red"')
        elif row1 == 1:
            results = cur.execute(sqlstr,("Reject",))
        elif row1 == 2:
            results = cur.execute(sqlstr,("Query",))
        elif row1 == 3:
            results = cur.execute('select * from Subcon_Info, Decisions where Decisions.Request_Pending_With = "EHC" and Decisions.Request_Status = "Pending" and Decisions.Decision_EHC IS NULL and Subcon_Info.Contract_Type = "T&M" and (Julianday(Subcon_Info.Expected_End_Date) - Julianday("now"))  between 0 and 30  and Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Cummulative_RAG = "Red"')
        else:
            results = cur.execute('select * from Subcon_Info, Decisions where (Decisions.Decision_SBU = "Approve" or Decisions.Decision_SBU = "Conditional Approval") and Subcon_Info.Contract_Type = "T&M" and (Julianday(Subcon_Info.Expected_End_Date) - Julianday("now"))  between 0 and 30  and Decisions.Subcon_ID = Subcon_Info.Subcon_ID  and Subcon_Info.Cummulative_RAG = "Red"')
        for row in results:
            for i in range(0,32):
                self.table.setItem(tablerow, i, QtWidgets.QTableWidgetItem(str(row[i])))
            tablerow+=1
    ...
    def ShowTotalLess(self, row1):  ## Show the info of all the Subcons under the Category, WO Expiry Date < 30 days
        connection = sqlite3.connect('EY')
        cur = connection.cursor()
        sqlstr = 'SELECT * FROM Subcon_Info, Decisions where Decisions.Decision_EHC = ? And (Julianday(Subcon_Info.Expected_End_Date) - Julianday("now"))  between 0 and 30  AND Decisions.Subcon_ID = Subcon_Info.Subcon_ID  and Subcon_Info.Cummulative_RAG = "Red"'
        tablerow=0
        if row1 == 0:
            results = cur.execute('SELECT * FROM Subcon_Info, Decisions where (Decisions.Decision_EHC = "Approve" or Decisions.Decision_EHC = "Conditional Approval") And (Julianday(Subcon_Info.Expected_End_Date) - Julianday("now"))  between 0 and 30  AND Decisions.Subcon_ID = Subcon_Info.Subcon_ID  and Subcon_Info.Cummulative_RAG = "Red"')
        elif row1 == 1:
            results = cur.execute(sqlstr,("Reject",))
        elif row1 == 2:
            results = cur.execute(sqlstr,("Query",))
        elif row1 == 3:
            results = cur.execute('select * from Subcon_Info, Decisions where Decisions.Request_Pending_With = "EHC" and Decisions.Request_Status = "Pending" and Decisions.Decision_EHC IS NULL and (Julianday(Subcon_Info.Expected_End_Date) - Julianday("now"))  between 0 and 30  and Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Cummulative_RAG = "Red"')
        else:
            results = cur.execute('select * from Subcon_Info, Decisions where (Decisions.Decision_SBU = "Approve" or Decisions.Decision_SBU = "Conditional Approval") and (Julianday(Subcon_Info.Expected_End_Date) - Julianday("now"))  between 0 and 30  and Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Cummulative_RAG = "Red"')
        for row in results:
            for i in range(0,32):
                self.table.setItem(tablerow, i, QtWidgets.QTableWidgetItem(str(row[i])))
            tablerow+=1
    ...
    def ShowFPMore(self, row1): ## Show the info of all the Subcons under the Category, Contract Type = FP and 30 days < WO Expiry Date < 60 days
        connection = sqlite3.connect('EY')
        cur = connection.cursor()
        sqlstr = 'SELECT * FROM Subcon_Info, Decisions where Subcon_Info.Contract_Type = "FP" AND Decisions.Decision_EHC = ? And ((Julianday(Subcon_Info.Expected_End_Date) - Julianday("now")) between 30 and 60) AND Decisions.Subcon_ID = Subcon_Info.Subcon_ID  and Subcon_Info.Cummulative_RAG = "Red"'
        tablerow=0
        if row1 == 0:
            results = cur.execute('SELECT * FROM Subcon_Info, Decisions where Subcon_Info.Contract_Type = "FP" AND (Decisions.Decision_EHC = "Approve" or Decisions.Decision_EHC = "Conditional Approval") And ((Julianday(Subcon_Info.Expected_End_Date) - Julianday("now")) between 30 and 60) AND Decisions.Subcon_ID = Subcon_Info.Subcon_ID  and Subcon_Info.Cummulative_RAG = "Red"')
        elif row1 == 1:
            results = cur.execute(sqlstr,("Reject",))
        elif row1 == 2:
            results = cur.execute(sqlstr,("Query",))
    ...
    def ShowTMMore(self, row1): ## Show the info of all the Subcons under the Category, Contract Type = T&M and 30 days < WO Expiry Date < 60 days
        connection = sqlite3.connect('EY')
        cur = connection.cursor()
        sqlstr = 'SELECT * FROM Subcon_Info, Decisions where Subcon_Info.Contract_Type = "T&M" AND Decisions.Decision_EHC = ? And ((Julianday(Subcon_Info.Expected_End_Date) - Julianday("now")) between 30 and 60) AND Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Cummulative_RAG = "Red" '
        tablerow=0
        if row1 == 0:
            results = cur.execute('SELECT * FROM Subcon_Info, Decisions where Subcon_Info.Contract_Type = "T&M" AND (Decisions.Decision_EHC = "Approve" or Decisions.Decision_EHC = "Conditional Approval") And ((Julianday(Subcon_Info.Expected_End_Date) - Julianday("now")) between 30 and 60) AND Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Cummulative_RAG = "Red" ')
        elif row1 == 1:
    ...
    def ShowTotalMore(self, row1): ## Show the info of all the Subcons under the Category, 30 days < WO Expiry Date < 60 days
        connection = sqlite3.connect('EY')
        cur = connection.cursor()
        sqlstr = 'SELECT * FROM Subcon_Info, Decisions where Decisions.Decision_EHC = ? And ((Julianday(Subcon_Info.Expected_End_Date) - Julianday("now")) between 30 and 60) AND Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Cummulative_RAG = "Red" '
        tablerow=0
        if row1 == 0:
            results = cur.execute('SELECT * FROM Subcon_Info, Decisions where (Decisions.Decision_EHC = "Approve" or Decisions.Decision_EHC = "Conditional Approval") And ((Julianday(Subcon_Info.Expected_End_Date) - Julianday("now")) between 30 and 60) AND Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Cummulative_RAG = "Red" ')
        elif row1 == 1:
            results = cur.execute(sqlstr,("Reject",))
        elif row1 == 2:
            results = cur.execute(sqlstr,("Query",))
            ...
            results = cur.execute('select * from Subcon_Info, Decisions where (Decisions.Decision_SBU = "Approve" or Decisions.Decision_SBU = "Conditional Approval") and ((Julianday(Subcon_Info.Expected_End_Date) - Julianday("now")) between 30 and 60) and Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Cummulative_RAG = "Red"')
        for row in results:
            for i in range(0,32):
                self.table.setItem(tablerow, i, QtWidgets.QTableWidgetItem(str(row[i])))
            tablerow+=1
    ...
    def loaddata_2(self):
        ...
        ## For Dashboard 1 - Adding values based on the Contract Type and WO Expiry date, and requests in the purview of EHC Approver ##
        sqlstr = 'select count(*) from Decisions, Subcon_Info where Decisions.Decision_EHC = ? AND Subcon_Info.Contract_Type = ? And ((Julianday(Subcon_Info.Expected_End_Date) - Julianday("now")) between 30 and 60) AND Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Cummulative_RAG = "Red"'
        sqlstr1 = 'select count(*) from Decisions, Subcon_Info where Decisions.Decision_EHC = ? AND Subcon_Info.Contract_Type = ? And (Julianday(Subcon_Info.Expected_End_Date) - Julianday("now"))  between 0 and 30  AND Decisions.Subcon_ID = Subcon_Info.Subcon_ID  and Subcon_Info.Cummulative_RAG = "Red"'
        
        cur.execute(sqlstr1, ("Approve", "FP", ))
        R1, = cur.fetchone()
        cur.execute(sqlstr1, ("Conditional Approval", "FP", ))
        R_1, = cur.fetchone()
        self.tableWidget1.setItem(0,1,QtWidgets.QTableWidgetItem(str(R1+R_1)))
        cur.execute(sqlstr1, ("Approve", "T&M",))
        R_2, = cur.fetchone()
        cur.execute(sqlstr1, ("Conditional Approval", "T&M",))
        R2, = cur.fetchone()
        ...
        cur.execute(sqlstr, ("Approve", "FP",))
        R3, = cur.fetchone()
        cur.execute(sqlstr, ("Conditional Approval", "FP",))
        R_3, = cur.fetchone()
        self.tableWidget4.setItem(0,1,QtWidgets.QTableWidgetItem(str(R3 + R_3)))
        cur.execute(sqlstr, ("Approve", "T&M",))
        R4, = cur.fetchone()
        cur.execute(sqlstr, ("Conditional Approval", "T&M",))
        R_4, = cur.fetchone()
        ...
        que1 = 'select count(*) from Subcon_Info, Decisions where Decisions.Request_Pending_With = "EHC" and Decisions.Request_Status = "Pending" and Decisions.Decision_EHC IS NULL and Contract_Type = ? and (Julianday(Subcon_Info.Expected_End_Date) - Julianday("now"))  between 0 and 30  and Subcon_Info.Cummulative_RAG = "Red" and Decisions.Subcon_ID = Subcon_Info.Subcon_ID'
        que2 = 'select count(*) from Subcon_Info, Decisions where Decisions.Request_Pending_With = "EHC" and Decisions.Request_Status = "Pending" and Decisions.Decision_EHC IS NULL and Contract_Type = ? and ((Julianday(Subcon_Info.Expected_End_Date) - Julianday("now")) between 30 and 60 and Subcon_Info.Cummulative_RAG = "Red" and Decisions.Subcon_ID = Subcon_Info.Subcon_ID)'
        
        cur.execute(que1,("FP",))
        R13, = cur.fetchone()
        self.tableWidget1.setItem(3,1,QtWidgets.QTableWidgetItem(str(R13)))
        cur.execute(que1, ("T&M",))
        R14, = cur.fetchone()
        ...
        results = cur.execute('Select Customer_Name, Contract_Type, Count(*), SUM(Vendor_Rate), SUM(Customer_Rate), Round(AVG(Julianday("now") - Julianday(Date_of_Joining)),0) from Subcon_Info group by Customer_Name, Contract_Type')
        tablerow = 0
        for row in results:
