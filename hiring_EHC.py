...
class EHC_hiring(object):
    def setupUi(self, MainWindow):
        ...
        self.Heading.setGeometry(QtCore.QRect(500,100, 800, 100))  ## Label for Heading
        font = QtGui.QFont()
        ...
        self.line_edit_1 = {}
        line_edit_1 = {
            "Unit": (1,1), "Sub Unit": (1,3),"Customer Name": (1,5),"Date of Joining": (3,1),"WO End Date": (3,3),"Customer Group Name": (1,7), "Project ID": (2,1), "Project Description": (2,3),"Project Contribution Margin (%)": (2,5), "Contract Type": (2,7), "Subcon Role": (3,5),"Subcon Band": (3,7),"Subcon Experience": (4,1),"Country": (4,5),	"Hiring Reason": (4,7),
    ...
    def Insert1(self):    ## On clicking the Submit Button 
        ...
        if self.Cummulative == "Red":   ## If the cummulative RAG of the person is Red
            if text1 == "Approve Request":  
                cur.execute('update Hiring set Decision_EHC = "Approve Request", Request_Status = "Approved", Transaction_Date_EHC = datetime("now"), Remarks_EHC = ? where Transaction_ID = ?',(text2,self.ID,))
            elif text1 == "Conditional Approval":
                cur.execute('update Hiring set Decision_EHC = "Conditional Approval", Request_Status = "Approved", Transaction_Date_EHC = datetime("now"), Remarks_EHC = ? where Transaction_ID = ?',(text2,self.ID,))
            elif text1 == "Send Back for Query":
                cur.execute('update Hiring set Decision_EHC = "Send Back for Query", Request_Status = "Pending", Request_Pending_With = "SBU Head",Transaction_Date_EHC = datetime("now"), Remarks_EHC = ? where Transaction_ID = ?',(text2,self.ID,))
            else:
                cur.execute('update Hiring set Decision_EHC = "Reject Request", Request_Status = "Rejected", Request_Pending_With = "",Transaction_Date_EHC = datetime("now"), Remarks_EHC = ? where Transaction_ID = ?',(text2,self.ID,))
        connection.commit()
        connection.close()
    ...
    def GoBack_3(self):  ## Go to Page 2
        self.transaction1_decision_RMG.setText("")
        self.transaction1_remarks_RMG.setText("")
        self.transaction1_date_RMG.setText("")
        self.transaction1_decision_SBU.setText("")
        ...
        self.transaction1_remarks_EHC.setText("")
        self.transaction1_date_EHC.setText("")
        self.stackedWidget.setCurrentIndex(1)  ## Move to page 1
    ...
    def showFP(self,row1):    ## For Dashboard containing info about the Contract Type - FP, clicking on a cell in row, different queries shall be executed
        connection = sqlite3.connect('EY')
        cur = connection.cursor()
        query1 = 'select * from Hiring where Contract_Type = "FP" and Decision_EHC = ?'
        if row1 == 1:  ## Query to show approved and conditionally approved requests
            results = cur.execute('select * from Hiring where Contract_Type = "FP" and (Decision_EHC = "Approve Request" or Decision_EHC = "Conditional Approval")')
        elif row1 == 2: ## Query to show rejected requests
            results = cur.execute(query1,("Reject Request",))
        elif row1 == 3: ## Query to show sent back requests
            results = cur.execute(query1,("Send Back for Query",))
        else:   ## Query to show unreviewed requests
            results = cur.execute('select * from Hiring where (Decision_SBU = "Approve Request" or Decision_SBU = "Conditional Approval") and Decision_EHC IS NULL and Contract_Type = "FP" and Request_Status = "Pending"')
        tablerow=0
    ...
    def showTM(self,row1):  ## For Dashboard containing info about the Contract Type - T&M, clicking on a cell in row, different queries shall be executed
        connection = sqlite3.connect('EY')   ## All the queries are similar to that in FP
        cur = connection.cursor()
        query1 = 'select * from Hiring where Contract_Type = "T&M" and Decision_EHC = ?'
        if row1 == 1:
            results = cur.execute('select * from Hiring where Contract_Type = "T&M" and (Decision_EHC = "Approve Request" or Decision_EHC = "Conditional Approval")')
        elif row1 == 2:
            results = cur.execute(query1,("Reject Request",))
        elif row1 == 3:
            results = cur.execute(query1,("Send Back for Query",))
        else:
            results = cur.execute('select * from Hiring where (Decision_SBU = "Approve Request" or Decision_SBU = "Conditional Approval") and Decision_EHC IS NULL and Contract_Type = "T&M" and Request_Status = "Pending"')
        tablerow=0
        for row in results:
            self.table.setItem(tablerow, 0, QtWidgets.QTableWidgetItem(str(row[0])))
            self.table.setItem(tablerow, 9, QtWidgets.QTableWidgetItem(str(row[37])))
    ...
    def FPall(self):   ## To show all the requests under the Contract Type FP and are under the purview of EHC
        connection = sqlite3.connect('EY')
        cur = connection.cursor()
        results = cur.execute('SELECT * FROM Hiring where Contract_Type = "FP"  and Decision_SBU = "Approved" and Cummulative_RAG = "Red"')
        tablerow=0
        for row in results:
            self.table.setItem(tablerow, 0, QtWidgets.QTableWidgetItem(str(row[0])))
            self.table.setItem(tablerow, 9, QtWidgets.QTableWidgetItem(str(row[37])))
    ...
    def TMall(self):   ## To show all the requests under the Contract Type T&M and are under the purview of EHC
        connection = sqlite3.connect('EY')
        cur = connection.cursor()
        results = cur.execute('SELECT * FROM Hiring where Contract_Type = "T&M" and Decision_SBU = "Approved" and Cummulative_RAG = "Red"')
        tablerow=0
        for row in results:
            self.table.setItem(tablerow, 0, QtWidgets.QTableWidgetItem(str(row[0])))
            self.table.setItem(tablerow, 9, QtWidgets.QTableWidgetItem(str(row[37])))
