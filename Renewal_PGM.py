## All the functions associated with PGM_Renewal have the same logic as the functions associated with SBU_Renewal. 
## The Logics have been listed in the Renewal_SBU file ##
...
class PGM_Renewal(object):
    def setupUi(self, MainWindow):
        ...
        self.table.setAlternatingRowColors(True)  ## Property to show Alternating Colors for Rows of table
        ...
        ## 2) Change the locations of others to fill its space. (Coordinates)
        ...
        self.transaction2_date_EHC = QtWidgets.QLabel(self.tab_4)
    ...
    def Insert1(self): ## On clicking the Submit button, the function inserts/update the info in the Decisions Table.
        ...
        if self.Cummulative == "Green": # If Cumulative RAG = Green, Approval or conditional approval implies Closing and Approving of Request
            if (self.text_2 == "Approve"):
                cur.execute('update Decisions set  Decision_PGM = "Approve", PGM_Remarks = ?, PGM_Extension_Tenure = ?, Request_Status = "Approved", Transaction_Date_PGM = datetime("now") where Subcon_ID = ? and Request_Status = "Pending"', (self.text_1,self.text_3,self.ID,))
            elif (self.text_2 == "Conditional Approval"):
                cur.execute('update Decisions set  Decision_PGM = "Conditional Approval", PGM_Remarks = ?, PGM_Extension_Tenure = ?, Request_Status = "Approved", Transaction_Date_PGM = datetime("now") where Subcon_ID = ? and Request_Status = "Pending"', (self.text_1,self.text_3,self.ID,))
            elif self.text_2 == "Query":  ## Query implies, Request will be pending with PM now.
                cur.execute('update Decisions set Decision_PGM = "Query", PGM_Remarks = ?, PGM_Extension_Tenure = ?, Request_Pending_With = ?, Transaction_Date_PGM = datetime("now") where Subcon_ID = ? and Request_Status = "Pending"', (self.text_1,self.text_3,"PM",self.ID,))
            else:  ## Rejected Requests means closing of request
                cur.execute('update Decisions set Decision_PGM = "Reject", PGM_Remarks = ?, PGM_Extension_Tenure = ?, Request_Status = "Rejected", Transaction_Date_PGM = datetime("now") where Subcon_ID = ? and Request_Status = "Pending"', (self.text_1,self.text_3,self.ID,))
        else: # If Cumulative RAG = Red, Approval or conditional approval implies Request is pending with EHC approver.
            if (self.text_2 == "Approve"):
                cur.execute('update Decisions set Decision_PGM = "Approve", PGM_Remarks = ?, PGM_Extension_Tenure = ?, Request_Pending_With = ?, Transaction_Date_PGM = datetime("now") where Subcon_ID = ? and Request_Status = "Pending"', (self.text_1,self.text_3,"SBU Head",self.ID,))
            elif  self.text_2 == "Conditional Approval":
                cur.execute('update Decisions set Decision_PGM = "Conditional Approval", PGM_Remarks = ?, PGM_Extension_Tenure = ?, Request_Pending_With = ?, Transaction_Date_PGM = datetime("now") where Subcon_ID = ? and Request_Status = "Pending"', (self.text_1,self.text_3,"SBU Head",self.ID,))
    ...
    def loaddata_1(self):
        connection = sqlite3.connect('EY')
        cur = connection.cursor()
        cur.execute('SELECT (Julianday("now") - Julianday("Date_of_Joining")) FROM Subcon_Info where Subcon_ID = ?', (self.ID,))## Calculating tenure. It is the difference in the number of days between today and Joining Day
        tenure, = cur.fetchone()
    ...
    def ShowFPLess(self, row1): ## Show the info of all the Subcons under the Category, Contract Type = FP and WO Expiry Date < 30 days
        connection = sqlite3.connect('EY')
        cur = connection.cursor()
        sqlstr = 'SELECT * FROM Subcon_Info, Decisions where Subcon_Info.Contract_Type = "FP" AND Decisions.Decision_PGM = ? And (Julianday(Subcon_Info.Expected_End_Date) - Julianday("now"))  between 0 and 30  AND Decisions.Subcon_ID = Subcon_Info.Subcon_ID  and Subcon_Info.Program_Manager = ?'
        
        if row1 == 0: # All the approved and conditionally approved requests
            cur.execute('SELECT count(*) FROM Subcon_Info, Decisions where Subcon_Info.Contract_Type = "FP" AND (Decisions.Decision_PGM = "Approve" or Decisions.Decision_PGM = "Conditional Approval") And (Julianday(Subcon_Info.Expected_End_Date) - Julianday("now"))  between 0 and 30  AND Decisions.Subcon_ID = Subcon_Info.Subcon_ID  and Subcon_Info.Program_Manager = ?',(self.name_PGM,))
            x, = cur.fetchone()
            self.rowcount = int(x)
            self.table.setRowCount(self.rowcount)  ## Setting the number of rows in the Table on Page-2 by counting the rows in the retrieved data
            results = cur.execute('SELECT * FROM Subcon_Info, Decisions where Subcon_Info.Contract_Type = "FP" AND (Decisions.Decision_PGM = "Approve" or Decisions.Decision_PGM = "Conditional Approval") And (Julianday(Subcon_Info.Expected_End_Date) - Julianday("now"))  between 0 and 30  AND Decisions.Subcon_ID = Subcon_Info.Subcon_ID  and Subcon_Info.Program_Manager = ?',(self.name_PGM,))
        elif row1 == 1: # All the rejected requests
            cur.execute('SELECT count(*) FROM Subcon_Info, Decisions where Subcon_Info.Contract_Type = "FP" AND Decisions.Decision_PGM = ? And (Julianday(Subcon_Info.Expected_End_Date) - Julianday("now"))  between 0 and 30  AND Decisions.Subcon_ID = Subcon_Info.Subcon_ID  and Subcon_Info.Program_Manager = ?',("Reject",self.name_PGM,))
            x, = cur.fetchone()
            self.rowcount = int(x)
            self.table.setRowCount(self.rowcount)
            results = cur.execute(sqlstr,("Reject",self.name_PGM,))
        elif row1 == 2: # All the requests sent back for query
            cur.execute('SELECT count(*) FROM Subcon_Info, Decisions where Subcon_Info.Contract_Type = "FP" AND Decisions.Decision_PGM = ? And (Julianday(Subcon_Info.Expected_End_Date) - Julianday("now"))  between 0 and 30  AND Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Program_Manager = ? ',("Query",self.name_PGM,))
            x, = cur.fetchone()
            self.rowcount = int(x)
            self.table.setRowCount(self.rowcount)
            results = cur.execute(sqlstr,("Query",self.name_PGM,))
        elif row1 == 3: # All the unattended requests under SBU Head
            cur.execute('select count(*) from Subcon_Info, Decisions where Decisions.Decision_PM = "Extend" and Decisions.Request_Status = "Pending" and Decisions.Decision_PGM IS NULL and Subcon_Info.Contract_Type = "FP" and (Julianday(Subcon_Info.Expected_End_Date) - Julianday("now"))  between 0 and 30  and Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Program_Manager = ?',(self.name_PGM,))
            x, = cur.fetchone()
            self.rowcount = int(x)
            self.table.setRowCount(self.rowcount)
            results = cur.execute('select * from Subcon_Info, Decisions where Decisions.Decision_PM = "Extend" and Decisions.Request_Status = "Pending" and Decisions.Decision_PGM IS NULL and Subcon_Info.Contract_Type = "FP" and (Julianday(Subcon_Info.Expected_End_Date) - Julianday("now"))  between 0 and 30  and Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Program_Manager = ?',(self.name_PGM,))
        else:  # all the requests under SBU Head
            cur.execute('select count(*) from Subcon_Info, Decisions where Decisions.Decision_PM = "Extend" and Subcon_Info.Contract_Type = "FP" and (Julianday(Subcon_Info.Expected_End_Date) - Julianday("now"))  between 0 and 30  and Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Program_Manager = ?',(self.name_PGM,))
            x, = cur.fetchone()
            self.rowcount = int(x)
            self.table.setRowCount(self.rowcount)
            results = cur.execute('select * from Subcon_Info, Decisions where Decisions.Decision_PM = "Extend" and Subcon_Info.Contract_Type = "FP" and (Julianday(Subcon_Info.Expected_End_Date) - Julianday("now"))  between 0 and 30  and Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Program_Manager = ?',(self.name_PGM,))
        tablerow=0
        for row in results:
            for i in range(0,32):
    ...
    def ShowTMLess(self, row1): ## Show the info of all the Subcons under the Category, Contract Type = T&M and WO Expiry Date < 30 days
        connection = sqlite3.connect('EY')
        cur = connection.cursor()
        sqlstr = 'SELECT * FROM Subcon_Info, Decisions where Subcon_Info.Contract_Type = "T&M" AND Decisions.Decision_PGM = ? And (Julianday(Subcon_Info.Expected_End_Date) - Julianday("now"))  between 0 and 30  AND Decisions.Subcon_ID = Subcon_Info.Subcon_ID  and Subcon_Info.Program_Manager = ?'
        sqlstr1 = 'SELECT count(*) FROM Subcon_Info, Decisions where Subcon_Info.Contract_Type = "T&M" AND Decisions.Decision_PGM = ? And (Julianday(Subcon_Info.Expected_End_Date) - Julianday("now"))  between 0 and 30  AND Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Program_Manager = ? '

        tablerow=0
        if row1 == 0:
            cur.execute('SELECT count(*) FROM Subcon_Info, Decisions where Subcon_Info.Contract_Type = "T&M" AND (Decisions.Decision_PGM = "Approve" or Decisions.Decision_PGM = "Conditional Approval") And (Julianday(Subcon_Info.Expected_End_Date) - Julianday("now"))  between 0 and 30  AND Decisions.Subcon_ID = Subcon_Info.Subcon_ID  and Subcon_Info.Program_Manager = ?',(self.name_PGM,))
            x, = cur.fetchone()
            self.rowcount = int(x)
            self.table.setRowCount(self.rowcount)
            results = cur.execute('SELECT * FROM Subcon_Info, Decisions where Subcon_Info.Contract_Type = "T&M" AND (Decisions.Decision_PGM = "Approve" or Decisions.Decision_PGM = "Conditional Approval") And (Julianday(Subcon_Info.Expected_End_Date) - Julianday("now"))  between 0 and 30  AND Decisions.Subcon_ID = Subcon_Info.Subcon_ID  and Subcon_Info.Program_Manager = ?',(self.name_PGM,))
        elif row1 == 1:
            cur.execute(sqlstr1,("Reject",self.name_PGM,))
            x, = cur.fetchone()
            self.rowcount = int(x)
            self.table.setRowCount(self.rowcount)
            results = cur.execute(sqlstr,("Reject",self.name_PGM,))
        elif row1 == 2:
            cur.execute(sqlstr1,("Query",self.name_PGM,))
            x, = cur.fetchone()
            self.rowcount = int(x)
            self.table.setRowCount(self.rowcount)
            results = cur.execute(sqlstr,("Query",self.name_PGM,))
        elif row1 == 3:
            cur.execute('select count(*) from Subcon_Info, Decisions where Decisions.Decision_PM = "Extend" and Decisions.Request_Status = "Pending" and Decisions.Decision_PGM IS NULL and Subcon_Info.Contract_Type = "T&M" and (Julianday(Subcon_Info.Expected_End_Date) - Julianday("now"))  between 0 and 30  and Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Program_Manager = ?',(self.name_PGM,))
            x, = cur.fetchone()
            self.rowcount = int(x)
            self.table.setRowCount(self.rowcount)
            results = cur.execute('select * from Subcon_Info, Decisions where Decisions.Decision_PM = "Extend" and Decisions.Request_Status = "Pending" and Decisions.Decision_PGM IS NULL and Subcon_Info.Contract_Type = "T&M" and (Julianday(Subcon_Info.Expected_End_Date) - Julianday("now"))  between 0 and 30  and Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Program_Manager = ?',(self.name_PGM,))
        else:
            cur.execute('select count(*) from Subcon_Info, Decisions where Decisions.Decision_PM = "Extend" and Subcon_Info.Contract_Type = "T&M" and (Julianday(Subcon_Info.Expected_End_Date) - Julianday("now"))  between 0 and 30  and Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Program_Manager = ?',(self.name_PGM,))
            x, = cur.fetchone()
            self.rowcount = int(x)
            self.table.setRowCount(self.rowcount)
            results = cur.execute('select * from Subcon_Info, Decisions where Decisions.Decision_PM = "Extend" and Subcon_Info.Contract_Type = "T&M" and (Julianday(Subcon_Info.Expected_End_Date) - Julianday("now"))  between 0 and 30  and Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Program_Manager = ?',(self.name_PGM,))
        for row in results:
            for i in range(0,32):
                self.table.setItem(tablerow, i, QtWidgets.QTableWidgetItem(str(row[i])))
            tablerow+=1
    ...
    def ShowTotalLess(self, row1): ## Show the info of all the Subcons under the Category, WO Expiry Date < 30 days
        connection = sqlite3.connect('EY')
        cur = connection.cursor()
        sqlstr = 'SELECT * FROM Subcon_Info, Decisions where Decisions.Decision_PGM = ? And (Julianday(Subcon_Info.Expected_End_Date) - Julianday("now"))  between 0 and 30  AND Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Program_Manager = ? '
        sqlstr1 = 'SELECT count(*) FROM Subcon_Info, Decisions where Decisions.Decision_PGM = ? And (Julianday(Subcon_Info.Expected_End_Date) - Julianday("now"))  between 0 and 30  AND Decisions.Subcon_ID = Subcon_Info.Subcon_ID  and Subcon_Info.Program_Manager = ?'
        
        tablerow=0
        if row1 == 0:
            cur.execute('SELECT count(*) FROM Subcon_Info, Decisions where (Decisions.Decision_PGM = "Approve" or Decisions.Decision_PGM = "Conditional Approval") And (Julianday(Subcon_Info.Expected_End_Date) - Julianday("now"))  between 0 and 30  AND Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Program_Manager = ? ',(self.name_PGM,))
            x, = cur.fetchone()
            self.rowcount = int(x)
            self.table.setRowCount(self.rowcount)
            ...
            cur.execute('select count(*) from Subcon_Info, Decisions where Decisions.Decision_PM = "Extend" and Decisions.Request_Status = "Pending" and Decisions.Decision_PGM IS NULL and (Julianday(Subcon_Info.Expected_End_Date) - Julianday("now"))  between 0 and 30  and Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Program_Manager = ?',(self.name_PGM,))
            x, = cur.fetchone()
            self.rowcount = int(x)
            self.table.setRowCount(self.rowcount)
            results = cur.execute('select * from Subcon_Info, Decisions where Decisions.Decision_PM = "Extend" and Decisions.Request_Status = "Pending" and Decisions.Decision_PGM IS NULL and (Julianday(Subcon_Info.Expected_End_Date) - Julianday("now"))  between 0 and 30  and Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Program_Manager = ?',(self.name_PGM,))
        else:
            cur.execute('select count(*) from Subcon_Info, Decisions where Decisions.Decision_PM = "Extend" and (Julianday(Subcon_Info.Expected_End_Date) - Julianday("now"))  between 0 and 30  and Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Program_Manager = ?',(self.name_PGM,))
            x, = cur.fetchone()
            self.rowcount = int(x)
            self.table.setRowCount(self.rowcount)
            results = cur.execute('select * from Subcon_Info, Decisions where Decisions.Decision_PM = "Extend" and (Julianday(Subcon_Info.Expected_End_Date) - Julianday("now"))  between 0 and 30  and Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Program_Manager = ?',(self.name_PGM,))
        for row in results:
            for i in range(0,32):
                self.table.setItem(tablerow, i, QtWidgets.QTableWidgetItem(str(row[i])))
            tablerow+=1
    ...
    def ShowFPMore(self, row1): ## Show the info of all the Subcons under the Category, Contract Type = FP and 30 days < WO Expiry Date < 60 days
        connection = sqlite3.connect('EY')
        cur = connection.cursor()
        sqlstr = 'SELECT * FROM Subcon_Info, Decisions where Subcon_Info.Contract_Type = "FP" AND (Decisions.Decision_PGM = "Approve" or Decisions.Decision_PGM = "Conditional Approval") And ((Julianday(Subcon_Info.Expected_End_Date) - Julianday("now")) between 30 and 60) AND Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Program_Manager = ? '
        sqlstr1 = 'SELECT count(*) FROM Subcon_Info, Decisions where Subcon_Info.Contract_Type = "FP" AND (Decisions.Decision_PGM = "Approve" or Decisions.Decision_PGM = "Conditional Approval") And ((Julianday(Subcon_Info.Expected_End_Date) - Julianday("now")) between 30 and 60) AND Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Program_Manager = ? '

        tablerow=0
        if row1 == 0:
            cur.execute('SELECT count(*) FROM Subcon_Info, Decisions where Subcon_Info.Contract_Type = "FP" AND Decisions.Decision_PGM = ? And ((Julianday(Subcon_Info.Expected_End_Date) - Julianday("now")) between 30 and 60) AND Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Program_Manager = ? ',(self.name_PGM,))
            x, = cur.fetchone()
            self.rowcount = int(x)
            self.table.setRowCount(self.rowcount)
            results = cur.execute('SELECT * FROM Subcon_Info, Decisions where Subcon_Info.Contract_Type = "FP" AND Decisions.Decision_PGM = ? And ((Julianday(Subcon_Info.Expected_End_Date) - Julianday("now")) between 30 and 60) AND Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Program_Manager = ? ',(self.name_PGM,))
        elif row1 == 1:
            cur.execute(sqlstr1,("Reject",self.name_PGM,))
            x, = cur.fetchone()
            self.rowcount = int(x)
            ...
            cur.execute('select count(*) from Subcon_Info, Decisions where Decisions.Decision_PM = "Extend" and Decisions.Request_Status = "Pending" and Decisions.Decision_PGM IS NULL and Subcon_Info.Contract_Type = "FP" and ((Julianday(Subcon_Info.Expected_End_Date) - Julianday("now")) between 30 and 60) and Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Program_Manager = ?',(self.name_PGM,))
            x, = cur.fetchone()
            self.rowcount = int(x)
            self.table.setRowCount(self.rowcount)
            results = cur.execute('select * from Subcon_Info, Decisions where Decisions.Decision_PM = "Extend" and Decisions.Request_Status = "Pending" and Decisions.Decision_PGM IS NULL and Subcon_Info.Contract_Type = "FP" and ((Julianday(Subcon_Info.Expected_End_Date) - Julianday("now")) between 30 and 60) and Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Program_Manager = ?',(self.name_PGM,))
        else:
            cur.execute('select count(*) from Subcon_Info, Decisions where Decisions.Decision_PM = "Extend" and Subcon_Info.Contract_Type = "FP" and ((Julianday(Subcon_Info.Expected_End_Date) - Julianday("now")) between 30 and 60) and Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Program_Manager = ?',(self.name_PGM,))
            x, = cur.fetchone()
            self.rowcount = int(x)
            self.table.setRowCount(self.rowcount)
            results = cur.execute('select * from Subcon_Info, Decisions where Decisions.Decision_PM = "Extend" and Subcon_Info.Contract_Type = "FP" and ((Julianday(Subcon_Info.Expected_End_Date) - Julianday("now")) between 30 and 60) and Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Program_Manager = ?',(self.name_PGM,))
        for row in results:
            for i in range(0,32):
                self.table.setItem(tablerow, i, QtWidgets.QTableWidgetItem(str(row[i])))
            tablerow+=1
    ...
    def ShowTMMore(self, row1):  ## Show the info of all the Subcons under the Category, Contract Type = T&M and 30 days < WO Expiry Date < 60 days
        connection = sqlite3.connect('EY')
        cur = connection.cursor()
        sqlstr = 'SELECT * FROM Subcon_Info, Decisions where Subcon_Info.Contract_Type = "T&M" AND Decisions.Decision_PGM = ? And ((Julianday(Subcon_Info.Expected_End_Date) - Julianday("now")) between 30 and 60) AND Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Program_Manager = ? '
        sqlstr1 = 'SELECT count(*) FROM Subcon_Info, Decisions where Subcon_Info.Contract_Type = "T&M" AND Decisions.Decision_PGM = ? And ((Julianday(Subcon_Info.Expected_End_Date) - Julianday("now")) between 30 and 60) AND Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Program_Manager = ? '
        
        tablerow=0
        if row1 == 0:
            cur.execute('SELECT * FROM Subcon_Info, Decisions where Subcon_Info.Contract_Type = "T&M" AND (Decisions.Decision_PGM = "Approve" or Decisions.Decision_PGM = "Conditional Approval") And ((Julianday(Subcon_Info.Expected_End_Date) - Julianday("now")) between 30 and 60) AND Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Program_Manager = ? ',(self.name_PGM,))
            x, = cur.fetchone()
            self.rowcount = int(x)
            self.table.setRowCount(self.rowcount)
            results = cur.execute('SELECT * FROM Subcon_Info, Decisions where Subcon_Info.Contract_Type = "T&M" AND (Decisions.Decision_PGM = "Approve" or Decisions.Decision_PGM = "Conditional Approval") And ((Julianday(Subcon_Info.Expected_End_Date) - Julianday("now")) between 30 and 60) AND Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Program_Manager = ? ',(self.name_PGM,))
            ...
            cur.execute('select count(*) from Subcon_Info, Decisions where Decisions.Decision_PM = "Extend" and Subcon_Info.Contract_Type = "T&M" and ((Julianday(Subcon_Info.Expected_End_Date) - Julianday("now")) between 30 and 60) and Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Program_Manager = ?',(self.name_PGM,))
            x, = cur.fetchone()
            self.rowcount = int(x)
            self.table.setRowCount(self.rowcount)
            results = cur.execute('select * from Subcon_Info, Decisions where Decisions.Decision_PM = "Extend" and Subcon_Info.Contract_Type = "T&M" and ((Julianday(Subcon_Info.Expected_End_Date) - Julianday("now")) between 30 and 60) and Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Program_Manager = ?',(self.name_PGM,))
        for row in results:
            for i in range(0,32):
                self.table.setItem(tablerow, i, QtWidgets.QTableWidgetItem(str(row[i])))
            tablerow+=1
    ...
    def ShowTotalMore(self, row1): ## Show the info of all the Subcons under the Category, 30 days < WO Expiry Date < 60 days
        connection = sqlite3.connect('EY')
        cur = connection.cursor()
        sqlstr = 'SELECT * FROM Subcon_Info, Decisions where Decisions.Decision_PGM = ? And ((Julianday(Subcon_Info.Expected_End_Date) - Julianday("now")) between 30 and 60) AND Decisions.Subcon_ID = Subcon_Info.Subcon_ID  and Subcon_Info.Program_Manager = ?'
        sqlstr1 = 'SELECT count(*) FROM Subcon_Info, Decisions where Decisions.Decision_PGM = ? And ((Julianday(Subcon_Info.Expected_End_Date) - Julianday("now")) between 30 and 60) AND Decisions.Subcon_ID = Subcon_Info.Subcon_ID  and Subcon_Info.Program_Manager = ?'
        tablerow=0
        if row1 == 0:
            cur.execute('SELECT * FROM Subcon_Info, Decisions where (Decisions.Decision_PGM = "Approve" or Decisions.Decision_PGM = "Conditional Approval") And ((Julianday(Subcon_Info.Expected_End_Date) - Julianday("now")) between 30 and 60) AND Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Program_Manager = ? ',(self.name_PGM,))
            x, = cur.fetchone()
            self.rowcount = int(x)
            self.table.setRowCount(self.rowcount)
            results = cur.execute('SELECT * FROM Subcon_Info, Decisions where (Decisions.Decision_PGM = "Approve" or Decisions.Decision_PGM = "Conditional Approval") And ((Julianday(Subcon_Info.Expected_End_Date) - Julianday("now")) between 30 and 60) AND Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Program_Manager = ? ',(self.name_PGM,))
            ...
            cur.execute('select * from Subcon_Info, Decisions where Decisions.Decision_PM = "Extend" and Decisions.Request_Status = "Pending" and Decisions.Decision_PGM IS NULL and ((Julianday(Subcon_Info.Expected_End_Date) - Julianday("now")) between 30 and 60) and Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Program_Manager = ?',(self.name_PGM,))
            x, = cur.fetchone()
            self.rowcount = int(x)
            self.table.setRowCount(self.rowcount)
            results = cur.execute('select * from Subcon_Info, Decisions where Decisions.Decision_PM = "Extend" and Decisions.Request_Status = "Pending" and Decisions.Decision_PGM IS NULL and ((Julianday(Subcon_Info.Expected_End_Date) - Julianday("now")) between 30 and 60) and Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Program_Manager = ?',(self.name_PGM,))
        else:
            cur.execute('select * from Subcon_Info, Decisions where Decisions.Decision_PM = "Extend" and ((Julianday(Subcon_Info.Expected_End_Date) - Julianday("now")) between 30 and 60) and Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Program_Manager = ?',(self.name_PGM,))
            x, = cur.fetchone()
            self.rowcount = int(x)
            self.table.setRowCount(self.rowcount)
            results = cur.execute('select * from Subcon_Info, Decisions where Decisions.Decision_PM = "Extend" and ((Julianday(Subcon_Info.Expected_End_Date) - Julianday("now")) between 30 and 60) and Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Program_Manager = ?',(self.name_PGM,))
        for row in results:
            for i in range(0,32):
                self.table.setItem(tablerow, i, QtWidgets.QTableWidgetItem(str(row[i])))
            tablerow+=1
    ...
    def loaddata_2(self):
        ...
        sqlstr = 'select count(*) from Decisions, Subcon_Info where Decisions.Decision_PGM = ? AND Subcon_Info.Contract_Type = ? And ((Julianday(Subcon_Info.Expected_End_Date) - Julianday("now")) between 30 and 60) AND Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Program_Manager = ?'
        sqlstr1 = 'select count(*) from Decisions, Subcon_Info where Decisions.Decision_PGM = ? AND Subcon_Info.Contract_Type = ? And (Julianday(Subcon_Info.Expected_End_Date) - Julianday("now"))  between 0 and 30  AND Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Program_Manager = ?'
        
        cur.execute(sqlstr1, ("Approve", "FP", self.name_PGM,))
        R1, = cur.fetchone() ## Approved Requests of FP and Expiry date < 30
        cur.execute(sqlstr1, ("Conditional Approval", "FP",self.name_PGM, ))
        ...
        cur.execute(sqlstr1, ("Approve", "T&M",self.name_PGM,))
        R_2, = cur.fetchone() ## Approved Requests of T&M and Expiry date < 30
        cur.execute(sqlstr1, ("Conditional Approval", "T&M",self.name_PGM,)) ## Conditionally Approved Requests of T&M and Expiry date < 30
        R2, = cur.fetchone()
        ...
        cur.execute(sqlstr, ("Approve", "FP",self.name_PGM,)) ## Approved Requests of FP and 30 < Expiry date < 60
        R3, = cur.fetchone()
        cur.execute(sqlstr, ("Conditional Approval", "FP",self.name_PGM,))
        R_3, = cur.fetchone()
        self.tableWidget4.setItem(0,1,QtWidgets.QTableWidgetItem(str(R3 + R_3)))
        cur.execute(sqlstr, ("Approve", "T&M",self.name_PGM,))
        R4, = cur.fetchone()
        cur.execute(sqlstr, ("Conditional Approval", "T&M",self.name_PGM,))
        R_4, = cur.fetchone()
        ...
        cur.execute(sqlstr1, ("Reject", "FP",self.name_PGM,)) ## Rejected Requests of FP and Expiry date < 30
        R5, = cur.fetchone()
        self.tableWidget1.setItem(1,1,QtWidgets.QTableWidgetItem(str(R5)))
        cur.execute(sqlstr1, ("Reject", "T&M",self.name_PGM,))
        R6, = cur.fetchone()
        ...
        cur.execute(sqlstr, ("Reject", "FP",self.name_PGM,)) ## Rejected Requests of FP and 30 < Expiry date < 60
        R7, = cur.fetchone()
        ...
        cur.execute(sqlstr1, ("Query", "FP",self.name_PGM,)) ## Sent Back for Query of FP and Expiry date < 30
        R9, = cur.fetchone()
        self.tableWidget1.setItem(2,1,QtWidgets.QTableWidgetItem(str(R9)))
        cur.execute(sqlstr1, ("Query", "T&M",self.name_PGM,))
        R10, = cur.fetchone()
        ...
        cur.execute(sqlstr, ("Query", "FP",self.name_PGM,)) ## Sent Back for Query of FP and 30 < Expiry date < 60
        R11, = cur.fetchone()
        self.tableWidget4.setItem(2,1,QtWidgets.QTableWidgetItem(str(R11)))
        cur.execute(sqlstr, ("Query", "T&M",self.name_PGM,))
        R12, = cur.fetchone()
        ...
        ## Logic: Decision by PM is Approve, Request Status is Pending, and There is no value in Decision_PGM
        que1 = 'select count(*) from Subcon_Info, Decisions where Decisions.Decision_PM = "Extend" and Decisions.Request_Status = "Pending" and Decisions.Decision_PGM IS NULL and Contract_Type = ? and (Julianday(Subcon_Info.Expected_End_Date) - Julianday("now"))  between 0 and 30  and Decisions.Subcon_ID = Subcon_Info.Subcon_ID and Subcon_Info.Program_Manager = ?'
        que2 = 'select count(*) from Subcon_Info, Decisions where Decisions.Decision_PM = "Extend" and Decisions.Request_Status = "Pending" and Decisions.Decision_PGM IS NULL and Contract_Type = ? and ((Julianday(Subcon_Info.Expected_End_Date) - Julianday("now")) between 30 and 60 and Decisions.Subcon_ID = Subcon_Info.Subcon_ID) and Subcon_Info.Program_Manager = ?'
        
        cur.execute(que1,("FP",self.name_PGM,)) ## No action initiated of FP and Expiry date < 30
        R13, = cur.fetchone()
        self.tableWidget1.setItem(3,1,QtWidgets.QTableWidgetItem(str(R13)))
        cur.execute(que1, ("T&M",self.name_PGM,))
        R14, = cur.fetchone()
        ...
        cur.execute(que2, ("FP",self.name_PGM,)) ## No action initiated of FP and 30 < Expiry date < 60
        R15, = cur.fetchone()
        self.tableWidget4.setItem(3,1,QtWidgets.QTableWidgetItem(str(R15)))
        cur.execute(que2, ("T&M",self.name_PGM,))
        R16, = cur.fetchone()
        ...
        results = cur.execute('Select Customer_Name, Contract_Type, Count(*), SUM(Vendor_Rate), SUM(Customer_Rate), Round(AVG(Julianday("now") - Julianday(Date_of_Joining)),0) from Subcon_Info group by Customer_Name, Contract_Type and Subcon_Info.Program_Manager = ?',(self.name_PGM,))
        tablerow = 0
        for row in results:
